...
def render(app: Dash, data: pd.DataFrame, id_barchart, dropdowns, x) -> html.Div:
    ...
    # x here: list of columns that should be plotted
    ...
    def update_bar_chart(*values: list[str]) -> html.Div:
        ...
        y = filtered_data[x].sum()

        fig = px.bar(x=x, y=y, labels={'x': 'End Use', 'y': 'Total demand'})
